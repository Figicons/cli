...
import { startLoading, endLoading } from "./logger.mjs";
...
export async function getFigmaProject(key) {
  startLoading(`🔎  Inspecting file with Figma API`);
  const figmaData = await getApiData(`files/${key}`);
  endLoading(`📚  %s Read file data from Figma`, "success");
...
export async function getImageData(figmaData) {
  ...
  startLoading(`🐕  Fetching ${icons.length} icons from Figma`);
